...
*/
    //Recursive Method for adding in a binary search tree
    public boolean add_recursive (E element)  
    {
        if (root == null) 
        {
            root = new TreeNode<E> (element, null);
        ...
        {
            TreeNode<E> temp = root;
            return add_recursive1(element,temp);

        }// root not null
    } // method add
    public boolean add_recursive1 (E element,TreeNode<E> temp) {
        int comp=0;
            ...
            if (comp < 0)
                if (temp.left != null)
                   // temp = temp.left;
                	add_recursive1(element,temp.left);
                else 
                {
                    temp.left = new TreeNode<E> (element, temp);
                    size++;      
                   ...
                   // temp = temp.right;
            		add_recursive1(element,temp.right);
                else 
                {
                    temp.right = new TreeNode<E> (element, temp);
                    size++;     
